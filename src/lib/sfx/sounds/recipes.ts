...
export const RECIPES = {
  ...
  /** A focused, bandpass-filtered tick with a bright sine ping on top. */
  tick: {
    masterGain: 0.4,
    layers: [
      {
  ...
  /** A papery filtered flick with a tiny glass tick. */
  page: {
    masterGain: 0.38,
    layers: [
      {
  ...
  /** A precise focus tick opening into a soft harmonic bloom. */
  ready: {
    masterGain: 0.45,
    layers: [
      {
...
export const SOUND_BLURBS: Record<SoundName, string> = {
  ...
  success: "Warm three-note confirmation — action succeeded",
  error: "Soft knock and descending refusal — recoverable errors",
  page: "Papery flick with a glass tick — pages, galleries",
  loading: "Brief unresolved rising shimmer — work starting",
  ready: "Focus tick with a harmonic bloom — content loaded",
};

/** Deep-clone a recipe for safe editing (e.g. in the /sfx lab). */
export function cloneRecipe(recipe: SoundRecipe): SoundRecipe {
  return structuredClone(recipe);
}

export function createBlankRecipe(): SoundRecipe {
  return {
    masterGain: 0.45,
    layers: [
      {
