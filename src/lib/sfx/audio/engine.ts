 ...
 * on one shared, lazily created `AudioContext`. Adapted from Cuelume
...
/**
 * Plays a named built-in sound. Safe to call from anywhere — lazily creates
 * the shared `AudioContext` on first use, and is a no-op when Web Audio is
